...
- **Auth (INSECURE — see Hazards):** the "token" is the employee's raw UUID sent as `Authorization: Bearer <uuid>`. No JWT, no signing, no expiry, no sessions. `lib/api-auth.ts` looks the UUID up directly. `lib/auth.ts` is a client-side localStorage shim (`AuthService`). Backdoors: `DEMO_AUTH=true` (hardcoded admin UUID), `ALLOW_DEFAULT_PASSWORD` / `password123` in `unified-login`.
...
## KNOWN BUGS (reported + root-caused, NOT yet fixed)
...
- **Security:** raw-UUID auth (anyone with a user's UUID = full impersonation), demo + default-password backdoors. ~40 of 151 routes have NO auth call at all — including **payroll** endpoints and an **unauthenticated auto-clock-in** (`app/api/verification/save-photo`). True cross-tenant leaks (return/write other tenants' data anonymously): `app/api/teams`, `app/api/quality-scores`, `GET /api/shifts` (helper `getShifts` drops the tenant filter), the entire `onboarding/*` group.
- **Broken SQL:** `lib/tenant-middleware.ts addTenantFilter()` — string-interpolates `tenantId` (SQL-injection) and regex-replaces every "where"; **exported, do not use**. `lib/database.ts getAttendanceSummary()` — aliases a table as `as` (reserved word), almost certainly broken at runtime.
