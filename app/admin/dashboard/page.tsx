...
export default function AdminDashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const managersResponse = await fetch('/api/employees?role=project_manager')
      let projectManagersData: any[] = []
      if (managersResponse.ok) {
        const data = await managersResponse.json()
        if (data.data) {
          projectManagersData = data.data
          setProjectManagers(projectManagersData)
          console.log(`📊 Loaded ${projectManagersData.length} project managers`)
        }
      }
  ...
  const handleAssignManager = async () => {
    if (!selectedProject || !selectedManagerId) {
      toast.error('Please select both project and manager')
      return
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast.error(data.error || 'Failed to assign project manager')
      }
    } catch (error) {
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Manage projects and assign project managers</CardDescription>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Select Project Manager</label>
                <select
