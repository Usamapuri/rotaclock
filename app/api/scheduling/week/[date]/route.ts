...
export async function GET(
  ...
  try {
    ...
    const locationId = searchParams.get('location_id') || '' // Add location filter
    ...
    let employeesQuery = `
      SELECT e.id, e.employee_code, e.first_name, e.last_name, e.email, e.department, e.job_position, e.location_id, l.name as location_name
      FROM employees e
      LEFT JOIN locations l ON e.location_id = l.id AND l.tenant_id = e.tenant_id
      WHERE e.is_active = true AND e.tenant_id = $1 AND e.role IN ('agent','employee')
    `
    const employeesParams: any[] = [tenantContext.tenant_id]

    // If user is a manager, restrict employees to their assigned location
    if (user.role === 'manager') {
      const idx = employeesParams.length + 1
      employeesQuery += ` AND e.location_id IN (
    ...
    // If location filter is provided (admin can filter by location)
    if (locationId && user.role === 'admin') {
      const idx = employeesParams.length + 1
      employeesQuery += ` AND e.location_id = $${idx}`
      employeesParams.push(locationId)
    }

    if (employeeId) {
      const idx = employeesParams.length + 1
      employeesQuery += ` AND e.id = $${idx}`
    ...
    // If manager, scope assignments to their location's employees
    if (user.role === 'manager') {
      assignmentsQuery += ` AND sa.employee_id IN (
        SELECT e.id FROM employees e
        WHERE e.tenant_id = $1 AND e.location_id IN (
          SELECT id FROM locations
          WHERE tenant_id = $1 AND manager_id = $${paramIndex}
        )
      )`
    ...
    // If location filter is provided (admin filtering by location)
    if (locationId && user.role === 'admin') {
      assignmentsQuery += ` AND sa.employee_id IN (
        SELECT id FROM employees
        WHERE tenant_id = $1 AND location_id = $${paramIndex}
      )`
      assignmentsParams.push(locationId)
      paramIndex++
    }
    ...
    if (!rotaId) {
      const rotasResult = await query(`
        ...
        ${user.role === 'manager' ? `AND r.location_id IN (
          SELECT id FROM locations
          WHERE tenant_id = $1 AND manager_id = $3
        )` : ''}
        ${locationId && user.role === 'admin' ? `AND r.location_id = $${user.role === 'manager' ? 4 : 3}` : ''}
        GROUP BY r.id
        ORDER BY r.created_at DESC
      `, user.role === 'manager' 
          ? [tenantContext.tenant_id, weekStartStr, user.id] 
          : (locationId && user.role === 'admin' ? [tenantContext.tenant_id, weekStartStr, locationId] : [tenantContext.tenant_id, weekStartStr]))
