...
export default function PMProjectsPage() {
  ...
  async function loadData() {
    try {
      ...
      // Load teams
      const teamsRes = await fetch('/api/project-manager/teams', {
        headers: { 'authorization': `Bearer ${user?.id || ''}` }
      })
  ...
  async function updateProject() {
    ...
    try {
      const user = AuthService.getCurrentUser()
      const res = await fetch(`/api/project-manager/projects/${selectedProject.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `Bearer ${user?.id || ''}`
  ...
  async function deleteProject(projectId: string) {
    ...
    try {
      const user = AuthService.getCurrentUser()
      const res = await fetch(`/api/project-manager/projects/${projectId}`, {
        method: 'DELETE',
        headers: { 'authorization': `Bearer ${user?.id || ''}` }
      })
  ...
  async function assignManager(projectId: string, managerId: string) {
    try {
      const user = AuthService.getCurrentUser()
      const res = await fetch('/api/project-manager/projects/assign-manager', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `Bearer ${user?.id || ''}`
