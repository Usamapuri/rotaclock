...
async function assignCurrentWeekShifts() {
  try {
    ...
    for (const date of weekDates) {
      ...
      // Randomly assign 1-2 shifts per employee per day
      for (const employee of employeesResult.rows) {
        const numShifts = Math.random() < 0.7 ? 1 : 2 // 70% chance of 1 shift, 30% chance of 2 shifts
        
        // Get random shifts for this employee
        const shuffledShifts = [...shiftsResult.rows].sort(() => Math.random() - 0.5)
        const selectedShifts = shuffledShifts.slice(0, numShifts)
