...
-- =====================================================
-- Version 2: Fixed for databases without proper PKs
-- Date: 2025-01-01
-- =====================================================
...
-- =====================================================
-- 0. FIX EMPLOYEES TABLE PRIMARY KEY (if needed)
-- =====================================================

-- Check and add primary key to employees if missing
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added PRIMARY KEY constraint to employees table';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added location_id column to employees';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added FK constraint: employees.location_id -> locations.id';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added approval_status to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE time_entries ADD COLUMN approved_by UUID REFERENCES employees(id) ON DELETE SET NULL;
    RAISE NOTICE 'Added approved_by to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added approved_at to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added rejection_reason to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added admin_notes to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added approved_hours to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added approved_rate to time_entries';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added total_pay to time_entries';
  END IF;
END$$;
...
BEGIN
  ...
  RAISE NOTICE '1. Run verification: npm run db:verify';
  RAISE NOTICE '2. Create location: npm run db:create-location';
