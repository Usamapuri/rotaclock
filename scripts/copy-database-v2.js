...
async function copyDatabase() {
  ...
  try {
    ...
    // Step 3: Drop all existing tables in target
    console.log('\n📋 Step 3: Dropping existing tables in target database...');
    ...
    // Step 4: Create tables without foreign key constraints first
    console.log('\n📋 Step 4: Creating tables (without foreign keys)...');
    ...
    console.log('\n📋 Step 5: Adding primary key constraints...');
    
    for (const tableName of tables) {
      const schema = schemas[tableName];
      for (const constraint of schema.constraints) {
        if (constraint.constraint_type === 'PRIMARY KEY') {
          console.log(`  Adding primary key to ${tableName}...`);
          try {
            ...
            console.log(`    Warning: Could not add primary key: ${err.message}`);
          }
        }
      }
    }
    ...
    for (const tableName of tables) {
      ...
      for (const constraint of schema.constraints) {
        if (constraint.constraint_type === 'FOREIGN KEY') {
          console.log(`  Adding foreign key to ${tableName}...`);
          try {
            await targetClient.query(`
              ALTER TABLE "${tableName}" 
              ADD CONSTRAINT "${constraint.constraint_name}" 
            ...
            console.log(`    Warning: Could not add foreign key: ${err.message}`);
          }
        } else if (constraint.constraint_type === 'UNIQUE') {
          console.log(`  Adding unique constraint to ${tableName}...`);
          try {
            await targetClient.query(`
              ALTER TABLE "${tableName}" 
              ADD CONSTRAINT "${constraint.constraint_name}" 
    ...
    // Step 9: Copy sequences and update their values
    console.log('\n📋 Step 9: Updating sequences...');
